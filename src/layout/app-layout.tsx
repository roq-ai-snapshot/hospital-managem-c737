...
import { useHospitalFindFirst } from 'lib/roq';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['medical-staff', 'hospital-administrator'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useHospitalFindFirst(
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Hospitals',
      path: isTenantUser ? `/hospitals/view/${data?.id}` : '/hospitals',
      entity: 'hospital',
    ...
    { name: 'Doctors', path: '/doctors', entity: 'doctor', service: AccessServiceEnum.PROJECT, icon: FiUserCheck },
    {
