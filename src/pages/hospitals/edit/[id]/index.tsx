...
import { useRoqClient, useHospitalFindFirst } from 'lib/roq';
...
function HospitalEditPage() {
  ...
  const { data, error, isLoading, mutate } = useHospitalFindFirst(queryParams, {}, { disabled: !id });
  ...
  const handleSubmit = async (values: HospitalInterface, { resetForm }: FormikHelpers<any>) => {
    ...
    try {
      ...
      const updated = await roqClient.hospital.update({
        data: values as RoqTypes.hospital,
        where: {
          id,
        },
      });
