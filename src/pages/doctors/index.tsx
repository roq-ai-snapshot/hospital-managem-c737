...
import { useRoqClient, useDoctorFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<DoctorInterface, unknown>;
...
export function DoctorListPage(props: DoctorListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'appointment.count', 'medical_record.count'],
  ...
  const { data, error, isLoading, mutate } = useDoctorFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.doctor.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('appointment', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('medical_record', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'medical_record',
          header: 'Medical Record',
          accessorKey: 'medical_record',
          cell: ({ row: { original: record } }: any) => record?._count?.medical_record,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/doctors/view/${record.id}`} passHref legacyBehavior>
                <Button
