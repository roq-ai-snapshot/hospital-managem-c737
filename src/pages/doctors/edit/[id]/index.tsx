...
import { useRoqClient, useDoctorFindFirst } from 'lib/roq';
...
function DoctorEditPage() {
  ...
  const { data, error, isLoading, mutate } = useDoctorFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: DoctorInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.doctor.update({
        data: values as RoqTypes.doctor,
        where: {
          id,
        },
      });
